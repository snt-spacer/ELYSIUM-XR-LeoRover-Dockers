...
## Description
This repostory contains the files to build a Docker image that contains the default ROS 2 package for the Realsense Camera. It currently uses the default `ros2_ws` of the robot to launch the `realsense_camera` ROS 2 package. The Docker used in this branch covers ROS 2 Jazzy and uses Zenoh as the communication middleware `rmw_zenoh_cpp`.
...
```bash
cd realsense_docker_zenoh/docker/
```
