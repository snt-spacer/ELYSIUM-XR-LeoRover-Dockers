# Leo Rover Livox MID 360 Docker

## Description
This repostory contains the files to build a Docker image that contains the ROS 2 package for the Livox MID 360 3D LiDAR, mounted on the Leo Rover. It currently uses the default `ros2_ws` of the robot to launch the `livox_ros2_driver` ROS 2 package. The Docker used in this branch covers ROS 2 Jazzy and is based on the Zenoh ROS2 Middleware `zenoh_rmw_cpp`.
