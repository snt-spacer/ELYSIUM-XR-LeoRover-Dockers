# RTAB-Map Livox Docker

## Description
This repostory contains the files to build a Docker image that contains the RTAB-Map ROS 2 package. It currently uses the default `ros2_ws` of the robot to launch the `rtabmap_livox` ROS 2 package. The Docker used in this branch covers ROS 2 Jazzy and is based on the Zenoh `rmw_zenoh_cpp` communication middleware.


## Getting started
Simply clone thie parent git repository:
...
```bash
cd rtabmap_livox_docker_zenoh/docker/
```
