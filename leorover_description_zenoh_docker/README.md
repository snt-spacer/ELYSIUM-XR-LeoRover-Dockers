# Leo Rover Description Docker

## Description
This repostory contains the files to build a Docker image that contains the ROS 2 package for the Leo Rover description for the ELYSIUM-XR project. It currently uses the default `ros2_ws` of the robot to launch the `leo_description` ROS 2 package. The Docker used in this branch covers ROS 2 Jazzy and is based on the Zenoh ROS2 Middleware `zenoh_rmw_cpp`.
...
```bash
cd leorover_description_zenoh_docker/docker/
```
...
The reason, the `ros2_ws` is mounted only when running the container is to allow modifying the workspace files, outside of the container. As a result, it is easily possible to modify the camera parameters without the need to rebuild the Docker image and copy the launch files into the image.
